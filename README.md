...
# spitroast
A very simple JavaScript monkeypatcher library, built into a userscript.

# Usage
```js
// Functions will be publicy available on the window object
window._spitroast;

const exampleObject = { testFunction: () => {} };

// Patches that run before the original function
window._spitroast.before("testFunction", exampleObject, (args) => { // `args` is an array of arguments passed to the original function
  console.log("Before");
...
window._spitroast.after("testFunction", exampleObject, (args, ret) => { // `ret` is the return value of the original function
  console.log("After");
...
const unpatch = window._spitroast.instead("testFunction", exampleObject, (args, orig) => { // `orig` is the original function itself
  console.log("Instead");
});
